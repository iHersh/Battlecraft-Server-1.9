...
public class FireworkJoin {
	...
	public void onPlayerJoin(final PlayerJoinEvent e) {
		...
		fm.addEffect(FireworkEffect.builder().flicker(true).trail(true).with(Type.BALL_LARGE).withColor(Color.BLUE, Color.RED, Color.WHITE).build());
		fm.setPower(3);
	...
	ArrayList<Player> cooldown = new ArrayList<Player>();
	 ...
	 final Player p = (Player) sender;
        ...
        if (cmd.getName().equalsIgnoreCase("fw")) {
     		...
     		p.sendMessage(ChatColor.GREEN + "Use " +ChatColor.YELLOW + "/firework " + ChatColor.GREEN + "to customize!");
     		cooldown.add(p);
     		Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
     			public void run() {
     				cooldown.remove(p);
     				p.sendMessage(BattlecraftServer.prefixMain + ChatColor.GREEN + "You can now recieve fireworks with " + ChatColor.YELLOW + "/fw!");
     			}
     		}, 3000);
