	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("invsee")) {
			...
			if (!p.hasPermission("bc.invsee")) {
				p.sendMessage(BattlecraftServer.prefixInv + ChatColor.RED + "You dont have permission to see another player's inventory!");
				return true;
			}
			...
			if (args.length == 1) {
				...
				p.sendMessage(BattlecraftServer.prefixInv + ChatColor.GREEN + tar.getName() + "'s inventory has been opened!");
				p.openInventory(inv);
				return true;
				}
			}
